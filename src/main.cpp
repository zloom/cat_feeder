...
#define FEED_PERIOD_SEC 8    // период кормёжки В ЧАСАХ
...
#define CLEAR_TIME 400   // время заднего хода, мс (устранение засора)
...
void feedRoutine()
{
  ...
  { // управление мотором
    tmr = millis();
    if (curEncSpeed < FEED_SPEED)
    {
      motorSpeed += 2; // прибавить газу
    }
    if (curEncSpeed > FEED_SPEED)
    {
      motorSpeed -= 2; // тормозить
    }

    motorSpeed = constrain(motorSpeed, 0, 255); // ограничить 0-255
    static bool stuckFlag = false;
    static uint32_t stuckTimeout;
    if (motorSpeed == 255 && curEncSpeed == 0)
    { // если мотор жарит на полную, но скорость 0
      if (millis() - stuckTimeout > 1000)
      {                   // и это продолжается больше секунды
        runMotor(1, 255); // полный назад! Прочищаем
        uint32_t stuckTmr = millis();
        while (millis() - stuckTmr < CLEAR_TIME)
        {            // в течение CLEAR_TIME
          encTick(); // опрос энкодера
        }
        //delay(CLEAR_TIME);
...
void isrHandler()
{
  if (!isrState)
  {
    isrState = true; // поднять флаг прерывания  если он сброшен
  }
}
...
void setup()
{
  ...
  sleedAmount = (float)FEED_PERIOD_SEC * 3600 / realPeriod * 1000; // находим сколько раз спать по 8 секунд!
#endif
...
void loop()
{
  if (isrState)
  {
    ...
    if (digitalRead(BTN_PIN) ^ INVERSE_BUTTON)
    {                   // если кнопка отпущена (клик)
      isrState = false; //
    }
    else
    {                              // если кнопка нажата
      digitalWrite(ENC_VCC, HIGH); // включаем энкодер
      while (!digitalRead(BTN_PIN) ^ INVERSE_BUTTON)
      {                // пока кнопка нажата
  ...
  if (!isrState)
  {                              // если проснулись по таймеру или кликнули по кнопке
    digitalWrite(ENC_VCC, HIGH); // включаем энкодер
    while (encCounter < feedAmount)
    { // пока не повернёмся нужное количество раз
      feedRoutine();
    }
    encCounter = 0;             // сброс счётчика
    motorSpeed = MIN_SPEED;     // скорость в мин
  ...
  { // спим по 8 секунд
    ...
    if (isrState)
      break; // если было нажатие - просыпаемся и уходим в начало loop
  }
#else
  // ИЛИ ОЖИДАНИЕ
  uint32_t tmr = millis();
  uint32_t waitPrd = (uint32_t)FEED_PERIOD_SEC * 1000;
  while (millis() - tmr < waitPrd)
  {
    if (isrState)
      break; // если было нажатие - выходим и уходим в начало loop
  }
#endif
}
